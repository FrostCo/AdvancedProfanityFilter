...
namespace filter {
// Globals
// declare let window: any; // TODO: Just adding to global/window context
declare const xpathDocText: '//*[not(self::script or self::style)]/text()[normalize-space(.) != ""]';
declare const xpathNodeText: './/*[not(self::script or self::style)]/text()[normalize-space(.) != ""]';
declare var whitespaceRegExp = new RegExp('\\s'); // TODO

declare var cfg: Config;
// window.config: Config;
declare var counter: 0;
declare var wordRegExps: RegExp[];
...
function disabledPage() {
  ...
  for (var x = 0; x < cfg.disabledDomains.length; x++) {
...
function generateRegexpList() {
  if (cfg.filterMethod == 2) { // Special regexp for "Remove" filter
    for (var x = 0; x < cfg.wordList.length; x++) {
...
function replaceText(str: string) {
  switch(cfg.filterMethod) {
    case 0: // Censor
      for (var z = 0; z < cfg.wordList.length; z++) {
